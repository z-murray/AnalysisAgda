...
record ℝ : Set where
  ...
  field
   ...
   -}
    ...
    n = zero
    n = suc m
    reg : ∀ (m n : ℕ) -> ℚ.∣ seq (suc m) - seq (suc n) ∣ ≤ ((+ 1) / (suc m)) + ((+ 1) / (suc n))
    -}
...
ℝ-Refl {x} (suc n) {n≢0} = lem ℚ.∣ mySeq ℚ.- mySeq ∣ 0ℚᵘ (mkℚᵘ (+ 2) n) (∣-∣-cong (+-inverseʳ mySeq)) (∣p∣≃p⇒0≤p ≃-refl)
  where
    mySeq : ℚᵘ
    mySeq = seq x (suc n)

    lem : ∀ (p q r : ℚᵘ) -> p ℚ.≃ q -> q ℚ.≤ r -> p ℚ.≤ r
    lem p q r p≃q q≤r = ≤-respˡ-≃ (≃-sym p≃q) q≤r

ℝ-Symm : Symmetric _≃_
ℝ-Symm {x} {y} x≃y (suc n) {n≢0} = ≤-respˡ-≃ (≃-sym lemE) (x≃y (suc n) {n≢0})
  where
    lemA : ℚ.- (seq y (suc n) ℚ.- seq x (suc n)) ℚ.≃ ℚ.- seq y (suc n) ℚ.- (ℚ.- seq x (suc n))
    lemA = ≃-reflexive (neg-distrib-+ (seq y (suc n)) (ℚ.- seq x (suc n)))

    lemB : ℚ.- seq y (suc n) ℚ.- (ℚ.- seq x (suc n)) ℚ.≃ ℚ.- seq y (suc n) ℚ.+ seq x (suc n)
    lemB = +-congʳ (ℚ.- seq y (suc n)) (neg-involutive (seq x (suc n)))

    lemC : ℚ.- seq y (suc n) ℚ.+ seq x (suc n) ℚ.≃ seq x (suc n) ℚ.- seq y (suc n)
    lemC = +-comm (ℚ.- seq y (suc n)) (seq x (suc n))

    lemD : ℚ.- (seq y (suc n) ℚ.- seq x (suc n)) ℚ.≃ seq x (suc n) ℚ.- seq y (suc n)
    lemD = ≃-trans (≃-trans lemA lemB) lemC

    lemE : ℚ.∣ seq y (suc n) ℚ.- seq x (suc n) ∣ ℚ.≃ ℚ.∣ seq x (suc n) ℚ.- seq y (suc n) ∣
    lemE = ≃-trans (≃-sym (∣-p∣≃∣p∣ (seq y (suc n) ℚ.- seq x (suc n)))) (∣-∣-cong lemD)
...
abstract
  ...
  archimedean-ℚi : ∀ (r : ℚᵘ) -> ∃ λ (N : ℕ) -> r ℚ.< (+ N) / 1
  archimedean-ℚi (mkℚᵘ p q-1) = 1 ℕ.+ ℤ.∣ t ∣ , ≤-<-trans (≤-reflexive part1) (≤-<-trans (≤-reflexive part2) (≤-<-trans (≤-reflexive part3)
                 ...
                 part2 : ((+ r) ℤ.+ (t ℤ.* (+ q))) / q ℚ.≃ ((+ r) / q) ℚ.+ ((t ℤ.* (+ q)) / q)
                 part2 = help (+ r) (t ℤ.* (+ q)) q

                 part3 : ((+ r) / q) ℚ.+ ((t ℤ.* (+ q)) / q) ℚ.≃ ((+ r) / q) ℚ.+ ((t ℤ.* (+ q)) / (1 ℕ.* q))
                 part3 = ≃-reflexive (cong (λ x -> ((+ r) / q) ℚ.+ x) (/-cong {t ℤ.* (+ q)} {q} {t ℤ.* (+ q)} {1 ℕ.* q} _≡_.refl (sym (ℕP.*-identityˡ q)) _ _))

                 --Very slow. The source is ℚ-CollapseR.
                 part4 : ((+ r) / q) ℚ.+ ((t ℤ.* (+ q)) / (1 ℕ.* q)) ℚ.≃ ((+ r) / q) ℚ.+ (t / 1)
                 part4 = +-congʳ ((+ r) / q) (ℚ-CollapseR t q 1)

                 part5 : ((+ r) / q) ℚ.+ (t / 1) ℚ.< ((+ q) / q) ℚ.+ (t / 1)
                 part5 = +-monoˡ-< (t / 1) {+ r / q} {+ q / q} (ℚ._<_.*<* (ℤP.*-monoʳ-<-pos q-1 {+ r} {+ q} (+<+ (n%ℕd<d p q))))

                 part6 : ((+ r) / q) ℚ.+ (t / 1) ℚ.< ((+ 1) / 1) ℚ.+ (t / 1)
                 ...
                 part7 : ((+ r) / q) ℚ.+ (t / 1) ℚ.< ((+ 1) / 1) ℚ.+ ((+ ℤ.∣ t ∣) / 1)
                 part7 = <-≤-trans part6 (+-monoʳ-≤ ((+ 1) / 1) {t / 1} {(+ ℤ.∣ t ∣) / 1} (*≤* part7a))

                 part8 : ((+ r) / q) ℚ.+ (t / 1) ℚ.< ((+ 1) ℤ.+ (+ ℤ.∣ t ∣)) / 1
...
-}
abstract
  complete-archimedean-ℚ : ∀ (p r : ℚᵘ) -> ℚ.Positive p -> ∃ λ (N : ℕ) -> r ℚ.< ((+ N) ℤ.* (↥ p)) / (↧ₙ p)
  complete-archimedean-ℚ (mkℚᵘ (+ p) q-1) (mkℚᵘ u v-1) p/q>0 = ℤ.∣ (+ 1) ℤ.+ t ∣ , part9
    where
      ...
      part9 : u / v ℚ.< ((+ ℤ.∣ (+ 1) ℤ.+ t ∣) ℤ.* (+ p)) / q
      part9 = ℚ._<_.*<* (ℤP.<-≤-trans (ℤP.<-≤-trans part6 part8) (ℤP.≤-reflexive (sym (ℤP.*-assoc (+ ℤ.∣ (+ 1) ℤ.+ t ∣) (+ p) (+ v)))))
...
lemma1B x y hyp (suc n) = lemA lemB
  where
    ∣xn-yn∣ : ℚᵘ
    ∣xn-yn∣ = ℚ.∣ seq x (suc n) ℚ.- seq y (suc n) ∣

    2/n : ℚᵘ
    2/n = (+ 2) / (suc n)
    
    lemA : (∀ (j : ℕ) -> {j≢0 : j ≢0} ->
          ℚ.∣ seq x (suc n) ℚ.- seq y (suc n) ∣ ℚ.≤ ((+ 2) / (suc n)) ℚ.+ ((+ 3) / j) {j≢0}) ->
          ℚ.∣ seq x (suc n) ℚ.- seq y (suc n) ∣ ℚ.≤ ((+ 2) / (suc n))
    -- Trichotomy of ℚ.
    ...
    -- xₙ - yₙ = (xₙ - zₙ) + (zₙ - yₙ). 
    lemA hyp with <-cmp ℚ.∣ seq x (suc n) ℚ.- seq y (suc n) ∣ ((+ 2) / (suc n))
    ... | tri< a ¬b ¬c = <⇒≤ a
    ... | tri≈ ¬a b ¬c = ≤-reflexive b
    ... | tri> ¬a ¬b c with complete-archimedean-ℚ (∣xn-yn∣ ℚ.- 2/n) ((+ 3) / 1) isPos
      where
    ...
    ... | suc M , 3<Nres = ⊥-elim (<-irrefl ≃-refl (<-≤-trans part4 (hyp N)))
      where
        ...
        part1 = <-respʳ-≃ (alternate (∣xn-yn∣ ℚ.- 2/n) N) 3<Nres

        part2a : ((+ 1) / N) ℚ.* ((+ N) / 1) ℚ.≃ ℚ.1ℚᵘ
        part2a = ℚ.*≡* (trans (ℤP.*-identityʳ ((+ 1) ℤ.* (+ N))) (sym (trans (ℤP.*-identityˡ ((+ N) ℤ.* (+ 1))) (ℤP.*-comm (+ N) (+ 1)))))

        part2b : ((+ 1) / N) ℚ.* (((+ N) / 1) ℚ.* (∣xn-yn∣ ℚ.- 2/n)) ℚ.≃ ∣xn-yn∣ ℚ.- 2/n
        part2b = ≃-trans (≃-sym (*-assoc ((+ 1) / N) ((+ N) / 1) (∣xn-yn∣ ℚ.- 2/n)))
                 (≃-trans (*-congʳ part2a) (ℚP.*-identityˡ (∣xn-yn∣ ℚ.- 2/n)))
    ...
    ... | N , proof = ≤-trans part1 (≤-trans part2 part3)
      where
        ...
        xn=xn-xm+xm : seq x (suc n) ℚ.≃ (seq x (suc n) ℚ.- seq x m) ℚ.+ seq x m
        ...
        -yn=-ym+ym-yn : ℚ.- seq y (suc n) ℚ.≃ (ℚ.- seq y m) ℚ.+ (seq y m ℚ.- seq y (suc n))
        ...
        xn+yn : seq x (suc n) ℚ.- seq y (suc n) ℚ.≃
                ((seq x (suc n) ℚ.- seq x m) ℚ.+ (seq x m ℚ.- seq y m)) ℚ.+ (seq y m ℚ.- seq y (suc n))
        xn+yn = ≃-trans (+-congˡ (ℚ.- seq y (suc n)) xn=xn-xm+xm)
                (≃-trans (+-congʳ ((seq x (suc n) ℚ.- seq x m) ℚ.+ seq x m) -yn=-ym+ym-yn)
                (≃-trans (≃-sym (+-assoc ((seq x (suc n) ℚ.- seq x m) ℚ.+ seq x m) (ℚ.- seq y m) (seq y m ℚ.- seq y (suc n))))
                (+-congˡ (seq y m ℚ.- seq y (suc n)) (+-assoc (seq x (suc n) ℚ.- seq x m) (seq x m) (ℚ.- seq y m)))))

        part1 : ℚ.∣ seq x (suc n) ℚ.- seq y (suc n) ∣ ℚ.≤
                ℚ.∣ seq x (suc n) ℚ.- seq x m ∣ ℚ.+ ℚ.∣ seq x m ℚ.- seq y m ∣ ℚ.+ ℚ.∣ seq y m ℚ.- seq y (suc n) ∣
        part1 = ≤-respˡ-≃ (∣-∣-cong (≃-sym xn+yn)) (≤-trans (∣p+q∣≤∣p∣+∣q∣ ((seq x (suc n) ℚ.- seq x m) ℚ.+ (seq x m ℚ.- seq y m)) (seq y m ℚ.- seq y (suc n)))
                                                   (+-monoˡ-≤ ℚ.∣ seq y m ℚ.- seq y (suc n) ∣ (∣p+q∣≤∣p∣+∣q∣ (seq x (suc n) ℚ.- seq x m) (seq x m ℚ.- seq y m))))

        part2 : ℚ.∣ seq x (suc n) ℚ.- seq x m ∣ ℚ.+ ℚ.∣ seq x m ℚ.- seq y m ∣ ℚ.+ ℚ.∣ seq y m ℚ.- seq y (suc n) ∣ ℚ.≤
                (((+ 1) / (suc n)) ℚ.+ ((+ 1) / m)) ℚ.+ ((+ 1) / (suc j)) ℚ.+ (((+ 1) / m) ℚ.+ ((+ 1) / (suc n)))
        part2 = ≤-trans (≤-trans
                        (+-monoˡ-≤ ℚ.∣ seq y m ℚ.- seq y (suc n) ∣ (+-monoˡ-≤ ℚ.∣ seq x m ℚ.- seq y m ∣ (reg x (suc n) m)))
                        (+-monoˡ-≤ ℚ.∣ seq y m ℚ.- seq y (suc n) ∣ (+-monoʳ-≤ (((+ 1) / (suc n)) ℚ.+ ((+ 1) / m))
                                           (proof m (ℕP.m≤m⊔n (suc N) (suc j))))))
                        (+-monoʳ-≤ ((((+ 1) / (suc n)) ℚ.+ ((+ 1) / m)) ℚ.+ ((+ 1) / (suc j))) (reg y m (suc n)))

        1/m≤1/j : ((+ 1) / m) ℚ.≤ (+ 1) / (suc j)
        1/m≤1/j = *≤* (ℤP.≤-trans (ℤP.≤-reflexive (ℤP.*-identityˡ (+ (suc j))))
                      (ℤP.≤-trans (+≤+ (ℕP.m≤n⊔m (suc N) (suc j)))
                      (ℤP.≤-reflexive (sym (ℤP.*-identityˡ (+ m))))))

        part3 : (((+ 1) / (suc n)) ℚ.+ ((+ 1) / m)) ℚ.+ ((+ 1) / (suc j)) ℚ.+ (((+ 1) / m) ℚ.+ ((+ 1) / (suc n))) ℚ.≤
                ((+ 2) / (suc n)) ℚ.+ ((+ 3) / (suc j))
        part3 = ≤-trans (+-monoʳ-≤ ((((+ 1) / (suc n)) ℚ.+ ((+ 1) / m)) ℚ.+ ((+ 1) / (suc j))) (+-monoˡ-≤ ((+ 1) / (suc n)) 1/m≤1/j))
                (≤-trans (+-monoˡ-≤ (((+ 1) / (suc j)) ℚ.+ ((+ 1) / (suc n))) (+-monoˡ-≤ ((+ 1) / (suc j)) (+-monoʳ-≤ ((+ 1) / (suc n)) 1/m≤1/j)))
                (≤-respˡ-≃ (+-congˡ (+ 1 / suc j ℚ.+ + 1 / suc n) (≃-sym (+-assoc ((+ 1) / (suc n)) ((+ 1) / (suc j)) ((+ 1) / (suc j)))))
                (≤-respˡ-≃ (+-congˡ (((+ 1) / (suc j)) ℚ.+ ((+ 1) / (suc n))) (+-congʳ ((+ 1) / (suc n)) (help (+ 1) (+ 1) (suc j))))
                (≤-respˡ-≃ (≃-sym (+-assoc ((+ 1) / (suc n)) ((+ 2) / (suc j)) (((+ 1) / (suc j)) ℚ.+ ((+ 1) / (suc n)))))
                (≤-respˡ-≃ (+-congʳ ((+ 1) / (suc n)) (+-assoc ((+ 2) / (suc j)) ((+ 1) / (suc j)) ((+ 1) / (suc n))))
                (≤-respˡ-≃ (+-congʳ ((+ 1) / (suc n)) (+-congˡ ((+ 1) / (suc n)) (help (+ 2) (+ 1) (suc j))))
                (≤-respˡ-≃ (+-comm (((+ 3) / (suc j)) ℚ.+ ((+ 1) / (suc n))) ((+ 1) / (suc n)))
  ...
  where
    ...
    lem (suc j) {j≢0} | N₁ , xy | N₂ , yz = N₁ ℕ.⊔ N₂ , partN
      ...
      -- Might prove a bunch of common analysis tricks generally as I go along to cut down on the algebra.
        partA : ∀ (n : ℕ) -> seq x n ℚ.- seq z n ℚ.≃ (seq x n ℚ.- seq z n) ℚ.+ 0ℚᵘ
        partA n = ≃-sym (+-identityʳ (seq x n ℚ.- seq z n))

        partB : ∀ (n : ℕ) -> (seq x n ℚ.- seq z n) ℚ.+ 0ℚᵘ ℚ.≃ (seq x n ℚ.- seq z n) ℚ.+ (seq y n ℚ.- seq y n)
        partB n = +-congʳ (seq x n ℚ.- seq z n) (≃-sym (+-inverseʳ (seq y n)))

        partC : ∀ (n : ℕ) -> (seq x n ℚ.- seq z n) ℚ.+ (seq y n ℚ.- seq y n) ℚ.≃ seq x n ℚ.+ ((ℚ.- seq z n) ℚ.+ (seq y n ℚ.- seq y n))
        partC n = +-assoc (seq x n) (ℚ.- seq z n) (seq y n ℚ.- seq y n)

        partD : ∀ (n : ℕ) -> seq x n ℚ.+ ((ℚ.- seq z n) ℚ.+ (seq y n ℚ.- seq y n)) ℚ.≃ seq x n ℚ.+ (((ℚ.- seq z n) ℚ.+ seq y n) ℚ.- seq y n)
        partD n = +-congʳ (seq x n) (≃-sym (+-assoc (ℚ.- seq z n) (seq y n) (ℚ.- seq y n)))

        partE : ∀ (n : ℕ) -> seq x n ℚ.+ (((ℚ.- seq z n) ℚ.+ (seq y n)) ℚ.- seq y n) ℚ.≃ seq x n ℚ.+ ((ℚ.- seq y n) ℚ.+ ((ℚ.- seq z n) ℚ.+ seq y n))
        partE n = +-congʳ (seq x n) (+-comm ((ℚ.- seq z n) ℚ.+ seq y n) (ℚ.- seq y n))

        partF : ∀ (n : ℕ) -> seq x n ℚ.+ ((ℚ.- seq y n) ℚ.+ ((ℚ.- seq z n) ℚ.+ seq y n)) ℚ.≃ (seq x n ℚ.- seq y n) ℚ.+ ((ℚ.- seq z n) ℚ.+ seq y n)
        partF n = ≃-sym (+-assoc (seq x n) (ℚ.- seq y n) ((ℚ.- seq z n) ℚ.+ seq y n))

        partG : ∀ (n : ℕ) -> (seq x n ℚ.- seq y n) ℚ.+ ((ℚ.- seq z n) ℚ.+ seq y n) ℚ.≃ (seq x n ℚ.- seq y n) ℚ.+ (seq y n ℚ.- seq z n)
        partG n = +-congʳ (seq x n ℚ.- seq y n) (+-comm (ℚ.- seq z n) (seq y n))

        partH : ∀ (n : ℕ) -> (seq x n ℚ.- seq z n) ℚ.≃ (seq x n ℚ.- seq y n) ℚ.+ (seq y n ℚ.- seq z n)
        partH n = ≃-trans (partA n) (≃-trans (partB n) (≃-trans (partC n) (≃-trans (partD n) (≃-trans (partE n) (≃-trans (partF n) (partG n))))))

        partI : numerator (((+ 1) / (2 ℕ.* (suc j))) ℚ.+ ((+ 1) / (2 ℕ.* (suc j)))) ≡ (+ 2) ℤ.* (+ (2 ℕ.* (suc j)))
        ...
        partL : (((+ 1) / (2 ℕ.* (suc j))) ℚ.+ ((+ 1) / (2 ℕ.* (suc j)))) ℚ.≃ (+ 1) / (suc j)
        partL = ≃-trans (≃-reflexive (/-cong partI _≡_.refl _ _)) (≃-trans partJ partK)

        partN : ∀ (n : ℕ) -> (N₁ ℕ.⊔ N₂) ℕ.< n -> ℚ.∣ seq x n ℚ.- seq z n ∣ ℚ.≤ ((+ 1) / (suc j))
        partN n N<n = ≤-respˡ-≃ (∣-∣-cong (≃-sym (partH n))) (≤-trans (∣p+q∣≤∣p∣+∣q∣ (seq x n ℚ.- seq y n) (seq y n ℚ.- seq z n))
                     (≤-trans (+-monoˡ-≤ ℚ.∣ seq y n ℚ.- seq z n ∣ (xy n (ℕP.m⊔n<o⇒m<o N₁ N₂ N<n)))
